...
def compute_layer_complete(
    ...
    for i, hidden_states in enumerate(inputs_embeds):
        ...
        if i == 1 and expert_layer_callback is not None:
...
class PaliGemmaWithExpertModel(
    ...
    def forward(
        ...
        if inputs_embeds[1] is None:
            ...
            if expert_layer_callback is not None and use_gradient_checkpointing:
                raise ValueError("PI0 TTT callbacks are not compatible with gradient checkpointing")

            # Process all layers with gradient checkpointing if enabled
            for layer_index, layers in enumerate(
                zip(paligemma_layers, gemma_expert_layers, strict=True)
            ):
        ...
        **kwargs: Unpack[ActionSelectKwargs],
        ...
        for step in range(num_steps):
            ...
            expert_layer_callback = (
                None
                if _expert_layer_callback_factory is None
                else _expert_layer_callback_factory(step == 0)
            )
