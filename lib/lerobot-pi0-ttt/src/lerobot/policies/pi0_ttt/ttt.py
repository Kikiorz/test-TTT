...
class TTTMLPLayer(nn.Module):
    """Update-then-apply TTT layer with a two-layer MLP fast model.

    The slow Q/K/V projections, fast-weight initialization, inner learning
    rate, and residual gate are learned by the outer task objective. One inner
    update is performed for every robot timestep, using all tokens from that
    timestep as the self-supervised mini-batch.
    """
    ...
    def forward(
        ...
        *,
        ...
        """Process ``[batch, timesteps, tokens, dim]`` and return the new state."""
        ...
        with (
            ...
            for timestep_inputs in projected_inputs.unbind(dim=1):
                ...
                outputs.append(timestep_inputs + torch.tanh(self.gate) * ttt_output)
