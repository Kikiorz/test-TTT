...
# Two-stage Method1 SmolVLA-TTT training for RMBench Cover Blocks.
#
# The canonical defaults use all 50 official demonstrations, tail-preserving
# 256-frame selected sequences, four GPUs, and per-device batch size 8. Every
...
Usage:
  train_cover_blocks_smolvla_ttt_two_stage.sh plan
  EXECUTE=1 train_cover_blocks_smolvla_ttt_two_stage.sh run

Useful overrides:
  BASE_CHECKPOINT=/path/to/native_smolvla/checkpoints/060000/pretrained_model
  DATASET_ROOT=/path/to/cover_blocks_demo_clean
  OUTPUT_ROOT=/path/to/rmbench_cover_blocks/smolvla_ttt_sequence_outer_v1_50x50
  BATCH_SIZE=8, NUM_PROCESSES=4, MAIN_PROCESS_PORT=29510
...
die() {
  echo "RMBench Cover Blocks SmolVLA-TTT preflight: $*" >&2
  exit 2
}
...
ENV_ID="RMBench-cover_blocks"
DATASET_REPO_ID="${DATASET_REPO_ID:-rmbench/cover_blocks_demo_clean}"
DATASET_ROOT="${DATASET_ROOT:-/workspace/data_rmbench_lerobot/cover_blocks_demo_clean}"
BASE_CHECKPOINT="${BASE_CHECKPOINT:-/workspace/experiments/rmbench_cover_blocks/native_smolvla_150ep/checkpoints/060000/pretrained_model}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/workspace/experiments/rmbench_cover_blocks/smolvla_ttt_sequence_outer_v1_50x50}"
...
print(
    json.dumps(
        {
            "schema": "method1_rmbench_smolvla_ttt_sequence_windows_v1",
...
PY
  )" || die "could not resolve the canonical 50-episode sequence epoch"
fi
...
' "${RESOLVED_STATS}")"
  ...
  EPISODES_JSON="<ALL_50_EPISODE_INDICES>"
  STAGE1_STEPS="<STEPS_PER_EPOCH*STAGE1_EPOCHS>"
  STAGE2_STEPS="<STEPS_PER_EPOCH*STAGE2_EPOCHS>"
fi
