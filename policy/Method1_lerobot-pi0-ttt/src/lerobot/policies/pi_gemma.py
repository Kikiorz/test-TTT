    ...
    class _PiGemmaDecoderLayerBase(GradientCheckpointingLayer):
        ...
        def forward(
            ...
            **kwargs,
            ...
            if post_attention_callback is not None:
                hidden_states = post_attention_callback(hidden_states)
        ...
        **kwargs,
        ...
        for layer_index, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            layer_outputs = decoder_layer(
                ...
                post_attention_callback=(
                    None
                    if layer_callback is None
                    else lambda hidden_states, index=layer_index: layer_callback(index, hidden_states)
                ),
                **kwargs,
            )
