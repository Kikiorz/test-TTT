...
class SmolVLATTTFlowMatching(nn.Module):
    ...
    def embed_suffix(self, noisy_actions, timestep):
        ...
        if self.register_tokens is not None:
            ...
            # helper additionally lets their queries read every action token.
            att_masks += [1] + [0] * (self.config.ttt_num_register_tokens - 1)

        # Keep action-action attention causal while allowing every action query
        # to read all preceding register tokens.
