    ...
    if metadata:
        ...
        if labeled_dataset.hd_window_local:
            if metadata.get("teacher_policy_type") != "smolvla_ttt":
                raise ValueError(
                    "HD window labels must be generated by a SmolVLA-TTT teacher; "
            ...
            if re.fullmatch(r"[0-9a-f]{64}", teacher_hash) is None:
                raise ValueError(
                    "HD window labels have an invalid teacher_config_sha256; "
            ...
            if teacher_layers is not None and [int(value) for value in teacher_layers] != expected_layers:
                raise ValueError(
                    "HD teacher/student TTT layer mismatch: "
                    f"teacher={teacher_layers}, student={expected_layers}"
                )
            teacher_registers = metadata.get("teacher_ttt_num_register_tokens")
            expected_registers = int(getattr(policy_cfg, "ttt_num_register_tokens", -1))
            if teacher_registers is not None and int(teacher_registers) != expected_registers:
                raise ValueError(
